package ec.backend.reactive.talentzone.song.routers;
...
import ec.backend.reactive.talentzone.song.usecases.*;
...
public class SongRouter {
    ...
    @Bean
    RouterFunction<ServerResponse> saveSongRouter(GetProductosByIdUseCase getProductosByIdUseCase, SaveSongUseCase saveSongUseCase){
        return route(POST("/song/save"),
                request -> request.bodyToMono(SongDTO.class)
                        .flatMap(songDTO -> getProductosByIdUseCase.apply(songDTO.getIdAlbum())
